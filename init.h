...
static char Buffer[512];  // 读数据块的缓冲区
...
static void reload_super_block(void);   // 加载超级块
static void update_group_desc(void);    // 更新组描述符
static void reload_group_desc(void);    // 加载组描述符
static void update_inode_entry(unsigned short i); // 更新inode项
static void reload_inode_entry(unsigned short i); // 加载inode项
...
static int get_inode(void); // 获取inode节点
static unsigned short reserch_file(char tmp[9],int file_type,unsigned short *inode_num,unsigned short *block_num,unsigned short *dir_num);// 搜索文件
static void dir_prepare(unsigned short tmp,unsigned short len,int type); // 准备目录项
static void remove_block(unsigned short del_num);// 删除数据块
static void remove_inode(unsigned short del_num);// 删除inode节点
static unsigned short search_file(unsigned short Ino);// 在文件打开表中查找是否已打开文件
static void sleep(int k);                // 延时函数
