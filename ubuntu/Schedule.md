...
|:---:|:---:|------|------|------|------|
| **1** | ❓ 待验证 | `Makefile`<br>`ext2_sim_disk.h`<br>`ext2_sim_fs.h` | 项目骨架：Kbuild 编译系统、磁盘数据结构（`__le16`标注）、内存结构（`sbi`、`inode_info`）、所有宏定义 | 三个文件编译通过 (`make` 零错误) | `make` → 生成 `ext2_sim.o`（未链接的空模块） |
