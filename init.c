...
static void update_super_block(void) // 更新超级块（写入磁盘）
{
    fp=fopen("./Ext2","r+");
    fseek(fp,DISK_START,SEEK_SET);
    fwrite(sb_block,SB_SIZE,1,fp);
    fflush(fp); // 刷新缓冲区，保证内存数据的一致性（注释重复行）
}

static void reload_super_block(void) // 加载超级块（从磁盘读取）
{
    fseek(fp,DISK_START,SEEK_SET);
    fread(sb_block,SB_SIZE,1,fp);// 读取数据到超级块缓冲区
}

static void update_group_desc(void) // 更新组描述符（写入磁盘）
{
    fp=fopen("./Ext2","r+");
...
static void reload_group_desc(void) // 加载组描述符（从磁盘读取）
{
...
static void update_inode_entry(unsigned short i) // 更新第i个inode项（写入磁盘）
{
    fp=fopen("./Ext2","r+");
...
static void reload_inode_entry(unsigned short i) // 加载第i个inode项（从磁盘读取）
{
    fseek(fp,INODE_TABLE+(i-1)*INODE_SIZE,SEEK_SET);
    fread(inode_area,INODE_SIZE,1,fp);
}

static void update_dir(unsigned short i) // 更新第i个数据块（作为目录项，写入磁盘）
{
    fp=fopen("./Ext2","r+");
...
static void reload_dir(unsigned short i) // 加载第i个数据块（作为目录项，从磁盘读取）
{
    fseek(fp,DATA_BLOCK+i*BLOCK_SIZE,SEEK_SET);
...
static void update_block_bitmap(void) // 更新块位图（写入磁盘）
{
    fp=fopen("./Ext2","r+");
...
static void reload_block_bitmap(void) // 加载块位图（从磁盘读取）
{
    fseek(fp,BLOCK_BITMAP,SEEK_SET);
    fread(bitbuf,BLOCK_SIZE,1,fp);
}

static void update_inode_bitmap(void) // 更新inode位图（写入磁盘）
{
    fp=fopen("./Ext2","r+");
...
static void reload_inode_bitmap(void) // 加载inode位图（从磁盘读取）
{
    fseek(fp,INODE_BITMAP,SEEK_SET);
    fread(ibuf,BLOCK_SIZE,1,fp);
}

static void update_block(unsigned short i) // 更新第i个数据块（写入磁盘）
{
    fp=fopen("./Ext2","r+");
    fseek(fp,DATA_BLOCK+i*BLOCK_SIZE,SEEK_SET);
    //fseek(fp,0,SEEK_SET);
...
static void reload_block(unsigned short i) // 加载第i个数据块（从磁盘读取）
{
    fseek(fp,DATA_BLOCK+i*BLOCK_SIZE,SEEK_SET);
...
static int alloc_block(void) // 分配一个数据块，返回数据块号
{
    ...
    // bitbuf共512个字节，表示4096个数据块。last_alloc_block/8确定bitbuf中哪个字节
    ...
    reload_block_bitmap();
    cur/=8;
    while(bitbuf[cur]==255)// 该字节的8个bit已全部分配
    {
        if(cur==511)cur=0; // 最后一个字节也已遍历完，从头开始寻找
        else cur++;
    }
    while(bitbuf[cur]&con) // 在该字节中找到具体的某一个bit为空闲
    {
        con=con/2;
        flag++;
    }
...
static int get_inode(void) // 获取一个空闲inode节点
{
    ...
    cur=(cur-1)/8;   // 注意：inode编号从1开始，存储从0开始
    //printf("%s",)
    while(ibuf[cur]==255) // 先检查该字节的8个位是否已全部占用
    {
        if(cur==511)cur=0;
        else cur++;
    }
    while(ibuf[cur]&con)  // 再检查该字节的某一位未被占用
    {
        con=con/2;
        flag++;
    }
...
// 在当前目录中搜索文件或目录名为tmp的项，获取其inode号、上级目录中的数据块号及目录项号
...
{
    unsigned short j,k;
    reload_inode_entry(current_dir); // 加载当前目录的inode
    j=0;
...
/* 为新建目录或文件准备 dir_entry
   对于普通文件：只需分配一个inode
   对于目录：除了分配inode外，还需要初始化数据块存储.和..两个目录项 */
...
static void remove_block(unsigned short del_num)
{
    ...
    tmp=del_num/8;
    reload_block_bitmap();
    switch(del_num%8) // 根据块号对8取余，将block位图中对应位置0
    {
...
static void remove_inode(unsigned short del_num)
{
    ...
    tmp=(del_num-1)/8;
    reload_inode_bitmap();
    switch((del_num-1)%8)// free the corresponding bit in inode bitmap
    {
        case 0:ibuf[tmp]=ibuf[tmp]&127;break;
        case 1:ibuf[tmp]=ibuf[tmp]&191;break;
        case 2:ibuf[tmp]=ibuf[tmp]&223;break;
        case 3:ibuf[tmp]=ibuf[tmp]&239;break;
        case 4:ibuf[tmp]=ibuf[tmp]&247;break;
        case 5:ibuf[tmp]=ibuf[tmp]&251;break;
        case 6:ibuf[tmp]=ibuf[tmp]&253;break;
        case 7:ibuf[tmp]=ibuf[tmp]&254;break;
    }
    update_inode_bitmap();
    gdt[0].bg_free_inodes_count++;
    update_group_desc();
...
//search if file is already opened in fopen_table
static unsigned short search_file(unsigned short Inode)
{
    unsigned short fopen_table_point=0;
    while(fopen_table_point<16)
    {
        if(fopen_table[fopen_table_point]==Inode)
            return 1;
        fopen_table_point++;
    }
    return 0;
...
void initialize_disk(void)  // 初始化虚拟磁盘
{
    int i=0;
    ...
    fp=fopen("./Ext2","w+"); // 创建虚拟磁盘文件，大小4612*512=2361344B
    fseek(fp,DISK_START,SEEK_SET);// 文件指针移到起始位置
    for(i=0;i<DISK_SIZE;i++)
    {
    	fwrite(Buffer,BLOCK_SIZE,1,fp); // 将文件全部清零（Buffer初始为全0）
    }
    // 初始化超级块
    reload_super_block();
    ...
    reload_dir(0);
    strcpy(current_path,"[root@ /");  // 修改路径为根目录
    // 初始化超级块
    reload_group_desc();

    gdt[0].bg_block_bitmap=BLOCK_BITMAP; // 块位图起始地址
    gdt[0].bg_inode_bitmap=INODE_BITMAP; // inode位图起始地址
    gdt[0].bg_inode_table=INODE_TABLE;   // inode表起始地址
    gdt[0].bg_free_blocks_count=DATA_BLOCK_COUNTS; // 空闲数据块总数
    gdt[0].bg_free_inodes_count=INODE_TABLE_COUNTS; // 空闲inode总数
    gdt[0].bg_used_dirs_count=0; // 初始时分配的目录节点数为0
    update_group_desc(); // 将组描述符写入磁盘
    ...
    inode_area[0].i_block[0]=alloc_block(); // 为根目录分配数据块
    //printf("%d f\n",inode_area[0].i_block[0]);
...
void cd(char tmp[9])
{
    ...
    {
        current_dir=i;
        if(!strcmp(tmp,"..")&&dir[k-1].name_len) /* 返回上一级目录且不是..目录 */
        {
...
void mkdir(char tmp[9],int type)
{
    ...
    {
        if(inode_area[0].i_size==4096) // 目录已满（4096字节=256个目录项）
        {
            printf("Directory has no room to be alloced!\n");
            return;
        }
        flag=1;
        if(inode_area[0].i_size!=inode_area[0].i_blocks*512) // 目录中某些块不满32个dir_entry
        {
            i=0;
...
void cat(char tmp[9],int type)
{
    ...
    if(!reserch_file(tmp,type,&i,&j,&k))
    {
        ...
        if(inode_area[0].i_size!=inode_area[0].i_blocks*512)
        {
            ...
            tmpno=dir[j].inode=get_inode();// 分配一个新的inode节点
            dir[j].name_len=strlen(tmp);
            dir[j].file_type=type;
            strcpy(dir[j].name,tmp);
            update_dir(inode_area[0].i_block[i-1]);
...
void rmdir(char tmp[9])
{
    ...
    if(flag)
    {
        reload_inode_entry(dir[k].inode); // 加载要删除目录的inode
        if(inode_area[0].i_size==32)  // 只有.和..两个目录项（即空目录）
        {
            inode_area[0].i_size=0;
            inode_area[0].i_blocks=0;
            ...
            /* 删除32个dir_entry全部为空的数据块
            其中inode_area[0].i_block[0]是目录.和..
            如果该数据块的非空dir_entry数量为0，则释放该块*/
            ...
            while(flag<32&&m<inode_area[0].i_blocks)
            {
                ...
                // 如果删除后某个数据块的目录项全部为空，释放该数据块并压缩数组
                if(flag==32)
                {
...
void del(char tmp[9])
{
    ...
    if(flag)
    {
        ...
        reload_inode_entry(i); // 加载要删除文件的inode
// 删除文件
        while(m<inode_area[0].i_blocks)
        {
        	remove_block(inode_area[0].i_block[m++]);
        ...
        reload_inode_entry(current_dir);
        reload_dir(inode_area[0].i_block[j]);
        dir[k].inode=0; // 清除目录项中的inode号

        update_dir(inode_area[0].i_block[j]);
        inode_area[0].i_size-=16;
        m=1;
        //if a whole directory block becomes empty free it
        while(m<inode_area[0].i_blocks)
        {
            flag=n=0;
            reload_dir(inode_area[0].i_block[m]);
...
void read_file(char tmp[9])
{
    ...
    if(flag)
    {
        if(search_file(dir[k].inode)) // 检查文件是否已打开
        {
            reload_inode_entry(dir[k].inode);
            // 检查是否有读权限
            if(!(inode_area[0].i_mode&4)) // i_mode低3位: 读(4),写(2),执行(1)
            {
            ...
            for(flag=0;flag<inode_area[0].i_blocks;flag++)
            {
                ...
                unsigned short bytes_in_block = inode_area[0].i_size - flag*512;
                if(bytes_in_block > 512) bytes_in_block = 512;
                for(t=0;t<bytes_in_block;++t)
                {
                	printf("%c",Buffer[t]);
                }
            }
...
// 以覆盖方式写入文件
void write_file(char tmp[9]) // 写入文件（覆盖写模式）
{
    ...
    if(flag)
    {
        ...
        {
            reload_inode_entry(dir[k].inode);
            if(!(inode_area[0].i_mode&2)) // i_mode低3位: 读(4),写(2),执行(1)
            {
            ...
            if(need_blocks<9) // 文件最多8个blocks(512bytes)
            {
                // 调整文件占用的数据块数量
                // 以覆盖方式写入，先判断原有的数据块数量
